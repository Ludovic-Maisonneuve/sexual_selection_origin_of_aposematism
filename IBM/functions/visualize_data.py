...
def save_data_for_mathematica(data_folder, Mathematica_data_folder, ngmin=0,
                              ngmax=None):  # Example usage: plot_mean_values_evolution("your_data_folder_path_here")

    if os.path.exists(Mathematica_data_folder):
        delete_folder(Mathematica_data_folder)

    # Use os.makedirs() to create the folder and any necessary parent directories
    os.makedirs(Mathematica_data_folder, exist_ok=True)
    ...
    for var, var_name in zip(list_vars, list_vars_name):
        with open(Mathematica_data_folder + '/' + var_name + ".json", 'w') as file:
            json.dump(locals()[f'list_{var_name}_mean'][imin:imax], file)

    with open(Mathematica_data_folder + '/gen.json', 'w') as file:
